"""Frozen classic Donchian/ATR trend development on seen perpetual data."""
...
from app.strategies.donchian_trend import DonchianTrendStrategy
...
VERSION = "perpetual-donchian-trend-development-v1"
TRAILING = TrailingStopPolicy(42, Decimal("3"))
CODE_FILES = (
    ...
    "app/strategies/donchian_trend.py", "app/derivatives/models.py",
    "app/derivatives/settings.py", "app/derivatives/risk.py", "app/derivatives/broker.py",
    "backtesting/derivatives.py", "backtesting/derivative_research.py",
    "backtesting/derivative_donchian_research.py",
)
...
def definition() -> dict:
    ...
    return {
        ...
        "hypothesis": (
            "A simple long-only 100-day trend and 20-day Donchian breakout, without "
            "efficiency or volume gates, produces enough independent opportunities for a "
            "targetless 3-ATR trailing exit to remain positive after funding and stress costs."
        ),
        "strategy": {"name": strategy.name, "version": strategy.version,
                     "parameters": json_value(asdict(strategy.parameters))},
        "control": "existing filtered long-only regime breakout with fixed 3R target",
        "candidate_exit": {"fixed_target": None, "atr_period": 42,
                           "atr_multiple": "3", "anchor": "highest completed close"},
        "cost_scenarios": ["current_conservative", "stress"], "cases": cases,
        "gate": (
            "Candidate positive in every year and cost, drawdown <10%, zero liquidations, "
            "maximum leverage <=10, at least 5 normal-cost and 3 stress trades in every year. "
            "No fallback."
        ),
        "rules": [
            "2023-2025 are seen development data; this study is not independent evidence.",
            "The 600-bar trend, 30-bar slope, 120-bar breakout and 42-bar ATR periods are fixed before evaluation.",
            "The candidate deliberately removes the old efficiency, ATR-fraction and volume entry gates.",
            "Initial and trailing stops are 3 ATR. No fixed profit target is used.",
            "Trailing updates use completed closes and become effective on the next candle only.",
            "Sizing, leverage, fees, slippage, mark-price liquidation and adverse funding remain unchanged.",
            "Exactly one new entry hypothesis and one unchanged control; no parameter sweep or post-result edit.",
...
def freeze(output: Path, datasets: list[Path], parent_results: Path) -> None:
    ...
    if (parent.get("assessment", {}).get("selected") is not None or not completion.is_file()
        ...
        raise ValueError("A completed failed trailing study is required")
...
def read_protocol(path: Path, datasets: list[Path]):
    ...
    if (protocol.get("protocol_version") != VERSION
        ...
        raise ValueError("Frozen Donchian protocol or source differs")
    for relative, expected in protocol["code_sha256"].items():
...
def assess(runs: list[dict]) -> dict:
    ...
    return {"primary": "classic_donchian_trailing", "checks": checks,
            "selected": "classic_donchian_trailing" if passed else None,
            "screen_passed": passed}


def render_report(result: dict) -> str:
    lines = ["# Klassischer Donchian-Trend mit ATR-Trailing", "",
             "2023-2025 sind vollständig gesehene Entwicklungsdaten. Das Ergebnis ist keine unabhängige Bestätigung.", "",
             "| Jahr | Variante | Fall | Trades | Netto USD | Profitfaktor | Max. DD | Funding |",
             "| --- | --- | --- | ---: | ---: | ---: | ---: | ---: |"]
    for run in result["runs"]:
        ...
        lines.append(f"| {run['year']} | {run['variant']} | {run['scenario']} | {p['trades']} | "
                     ...
                     f"{Decimal(p['max_drawdown']) * 100:.2f}% | {Decimal(p['funding_paid']):.2f} |")
...
def evaluate(path: Path, datasets: list[Path]) -> None:
    ...
    variants = {"fixed_3r_control": (LongOnlyRegimeStrategy, None),
                "classic_donchian_trailing": (DonchianTrendStrategy, TRAILING)}
    runs = []
    for year in (2023, 2024, 2025):
    ...
    if read_protocol(path, datasets)[0] != protocol:
        raise ValueError("Donchian protocol changed during evaluation")
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="Frozen classic Donchian perpetual development")
    ...
    try:
        ...
        print(f"Donchian research failed: {exc}", file=sys.stderr); return 1
