"""Frozen horizon-aligned short-term momentum study on seen 2023-2025 data."""
...
VERSION = "perpetual-short-horizon-tsmom-development-v1"
...
CODE_FILES = (
    ...
    "app/strategies/time_series_momentum.py", "app/strategies/short_term_momentum.py",
    ...
    "backtesting/derivative_short_momentum_research.py",
)
...
def definition(observed: ObservedCostScenario) -> dict:
    ...
    return {
        ...
        "hypothesis": (
            "Agreement between fixed one-week and four-week own-price momentum, "
            "paired with a one-week maximum holding horizon, captures the short-term "
            "crypto momentum reported over one-to-eight-week horizons after realistic costs."
        ),
        "research_basis": [
            "Liu and Tsyvinski (2018; RFS 2021), Risks and Returns of Cryptocurrency.",
            "The paper reports cryptocurrency time-series momentum over one-to-eight-week horizons.",
        ],
        ...
        "exit": {"maximum_holding_bars": 42, "fixed_target": None,
                 "protective_stop": "ATR(42) x 3", "execution": "next candle open"},
        ...
        "rules": [
            ...
            "Exactly one parameter set: 42/168-bar momentum, ATR(42), 3-ATR initial stop.",
            "Every open trade exits after at most 42 evaluation bars; no trailing or fixed target.",
            "No long-only switch, breakout, volume, oscillator, machine-learning or funding-timing filter.",
            ...
            "The earlier 30/120-day momentum test counts as a separate failed family member.",
...
def read_protocol(path: Path, datasets: list[Path], candidate: Path, summary: Path):
    ...
    if (protocol.get("protocol_version") != VERSION
        ...
        raise ValueError("Frozen short-horizon protocol, source or costs differ")
    for relative, expected in protocol["code_sha256"].items():
...
def assess(runs: list[dict]) -> dict:
    ...
    for year in (2023, 2024, 2025):
        for scenario in ("observed_p95", "double_cost_stress"):
            ...
            checks[f"{year}/{scenario}"] = {
                ...
                "enough_trades": performance["trades"] >= 12,
    ...
    return {"primary": "short_horizon_tsmom", "checks": checks,
            "selected": "short_horizon_tsmom" if passed else None,
            "screen_passed": passed}


def render_report(result: dict) -> str:
    lines = [
        "# Kurzfristiges Zeitreihen-Momentum", "",
...
def evaluate(path: Path, datasets: list[Path], candidate: Path, summary: Path) -> None:
    ...
    for year in (2023, 2024, 2025):
        ...
        for scenario, (configured, funding) in cost_cases(observed).items():
            replay = DerivativeBacktester(
                ShortTermMomentumStrategy(), configured, funding,
    ...
    if read_protocol(path, datasets, candidate, summary)[0] != protocol:
        raise ValueError("Short-horizon protocol changed during evaluation")
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="Frozen short-horizon TSMOM development")
    ...
    try:
        ...
        print(f"Short-horizon research failed: {exc}", file=sys.stderr)
        return 1
